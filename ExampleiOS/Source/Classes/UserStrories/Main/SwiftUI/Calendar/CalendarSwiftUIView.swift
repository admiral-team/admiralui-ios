...
struct CalendarSwiftUIView: View {
    ...
    private var startDate = Calendar.current.date(byAdding: .year, value: -2, to: Date()) ?? Date()
    private var endDate = Calendar.current.date(byAdding: .year, value: 2, to: Date()) ?? Date()
