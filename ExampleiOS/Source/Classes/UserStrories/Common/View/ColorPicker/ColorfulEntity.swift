...
extension HSVColor {
    init(color: UIColor, colorSpace: HRColorSpace) {
        ...
        switch colorSpace {
        case .extendedSRGB:
            cgColorSpace = CGColorSpace(name: CGColorSpace.displayP3) !! fatalError("")
        case .sRGB:
            cgColorSpace = CGColorSpace(name: CGColorSpace.sRGB) !! fatalError("")
        }

        let converted = color.cgColor.converted(to: cgColorSpace, intent: .defaultIntent, options: nil) !! fatalError("")
        let components: [CGFloat] = converted.components !! fatalError("")
