...
final class UploadImageViewSnapshotTests: XCTestCase {
    ...
    func createUploadImageView(status: ChatStatus) -> some View {
        let view = UploadImageView(model: UploadImageModel(time: "12:53", backgroundImage: Image(uiImage: TestsAsset.Chat.photo.image), uploadStatus: status), direction: .left)
        return view
    }
