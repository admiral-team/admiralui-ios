//
//  CalendarGhostButtonStyle.swift
...
/**
 ...
 ```
Button("Text", action: {})
    .buttonStyle(CalendarGhostButtonStyle(sizeType: .small))
 ```
 ...
 You can create a button with an activity indicator instead of text by specifying isLoading: .constant(true) in init CalendarGhostButtonStyle. In this case, the text that you pass to the Button will not be shown, but the activity indicator will be shown instead:

 # Code
 ```
 Button("Text", action: {})
    .buttonStyle(CalendarGhostButtonStyle(isLoading: .constant(true)))
 ```
*/
@available(iOS 14.0.0, *)
public struct CalendarGhostButtonStyle: ButtonStyle {

    // MARK: - Public Properties

    /// The loading flag of CalendarGhostButton
    @Binding public var isLoading: Bool

    /// The size type of GhostButton
    public var sizeType: CalendarButtonSizeType?

    // MARK: - Private Properties

    @ObservedObject private var schemeProvider: SchemeProvider<CalendarGhostButtonScheme>

    // MARK: - Inializer

    public init(
        isLoading: Binding<Bool> = .constant(false),
        sizeType: CalendarButtonSizeType? = nil,
        schemeProvider: SchemeProvider<CalendarGhostButtonScheme> = AppThemeSchemeProvider<CalendarGhostButtonScheme>()
    ) {
...
@available(iOS 14.0.0, *)
private extension CalendarGhostButtonStyle {
    struct GhostButton: View {
        ...
        var sizeType: CalendarButtonSizeType?
        let configuration: Configuration

        private var schemeProvider: SchemeProvider<CalendarGhostButtonScheme>

        // MARK: - Initializer

        init(
            isLoading: Binding<Bool>,
            schemeProvider: SchemeProvider<CalendarGhostButtonScheme>,
            sizeType: CalendarButtonSizeType?,
            configuration: Configuration
        ) {
        ...
        var body: some View {
            ...
            let content = isLoading ?
                CalendarActivityIndicator(style: .contrast, size: .medium).eraseToAnyView()
        ...
        private func contentButton(
            scheme: CalendarGhostButtonScheme,
            content: AnyView
        ) -> some View {
...
@available(iOS 14.0, *)
struct CalendarGhostButtonStyle_Previews: PreviewProvider {

    static var previews: some View {
        Button("Text", action: {})
            .buttonStyle(CalendarGhostButtonStyle(isLoading: .constant(false)))
    }
}
