//
//  CalendarActivityIndicator.swift
...
import SwiftUI
/**
 CalendarActivityIndicator - A view that shows that a task is in progress.
 You can create a CalendarActivityIndicator by specifying the following parameters in the initializer
 ...
 # Code
 ```
 CalendarActivityIndicator(style: .default, size: .small)
 ```
*/
@available(iOS 14.0, *)
public struct CalendarActivityIndicator: View {
    ...
    @ObservedObject var schemeProvider: SchemeProvider<CalendarActivityIndicatorScheme>
    ...
    public init(
        style: Style = .default,
        size: Size = .medium,
        schemeProvider: SchemeProvider<CalendarActivityIndicatorScheme> = AppThemeSchemeProvider<CalendarActivityIndicatorScheme>()
    ) {
        self.style = style
        self.size = size
