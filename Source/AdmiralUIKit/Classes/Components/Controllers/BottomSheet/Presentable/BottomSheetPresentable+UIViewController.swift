//
//  BottomSheetPresentable+UIViewController.swift
//  AdmiralUI
//
...
/// Extends PanModalPresentable with helper methods  when the conforming object is a UIViewController
public extension BottomSheetPresentable where Self: UIViewController {
    ...
    func panModalTransition(to state: BottomSheetPresentationState) {
    ...
    func panModalAnimate(_ animationBlock: @escaping AnimationBlockType, _ completion: AnimationCompletionType? = nil) {
        BottomSheetModalAnimator.animate(animationBlock, config: self, completion)
    }
